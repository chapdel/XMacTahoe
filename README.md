...
macOS 27 (Golden Gate) walks back some of Tahoe's excesses, and those changes are available here as a preset: `xmactahoe goldengate on` tints the glass (level 55 instead of full translucency), takes the corner radius from 18 px down to 12, dims inactive windows slightly so the active one stands out and sharpens the glass edges. `xmactahoe goldengate off` restores the Tahoe values, and `status` prints them.

The **window buttons are your own choice**, not part of the preset: `xmactahoe buttons glass` gives the traffic lights the Liquid Glass shading of macOS 27 (gradient, specular highlight, darkened when pressed), `flat` the flat discs of Tahoe that the package ships, and `auto` hands the decision back to the preset (glass with Golden Gate, flat with Tahoe). Once you pick one, the preset leaves the buttons alone. An inactive window keeps flat grey buttons either way, as on macOS.
